					...
					{() => (
						<>
							...
							<ModalBody className="px-[16px] pb-[16px] items-center gap-0">
								...
								<div className="text-[17px] text-[#fff] mt-[10px]">{info?.symbol?.toUpperCase() || '--'}</div>
								...
								<Button fullWidth className="h-[44px] bg-[#333] text-[15px] text-[#fff] rounded-[16px] mt-[12px] hover:bg-[#444] transition-colors" onPress={() => { copy(`https://popmefun.com/token/${info?.mint}` || '') }}>复制链接</Button>
							</ModalBody>
						</>
					)}
				</ModalContent>
