...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    // 1. 获取token总数
    const totalTokens = Number(await readContract(config, {
    ...
    for (let i = 0; i < matchedAddresses.length; i++) {
      ...
      if (tokenInfo) {
        ...
        if (processedInfo && processedInfo.reserve1 && processedInfo.target) {
          try {
            const reserve = parseFloat(processedInfo.reserve1);
            const target = parseFloat(processedInfo.target);
            if (target > 0) {
              progress = (reserve / target) * 100;
              progress = Math.min(progress, 100);
            }
          } catch (error) {
            console.warn(`计算进度失败 ${address}:`, error);
          }
        }
